...
if [[ $LOAD_IMAGES ]]; then
  ...
  for s in entity-attribute-service key-access-service entitlement-service storage-service atttributes-service; do
...
if [[ $LOAD_SECRETS ]]; then
  ...
  kubectl create secret generic atttributes-secrets --from-literal=POSTGRES_PASSWORD=myPostgresPassword || e "create aa secrets failed"
