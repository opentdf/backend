...
# builds
docker_build('virtru/tdf-attribute-authority-service', 'containers/service_attribute_authority')
...
docker_build('tdf3.python.base', 'containers/python_base')
...
helm_remote('postgresql', repo_url='https://charts.bitnami.com/bitnami', release_name='tdf', values=['deployments/docker-desktop/tdf-postgresql-values.yaml'])

# helm charts
# usage https://docs.tilt.dev/helm.html#helm-options
k8s_yaml(helm('charts/attribute_authority', 'attribute-authority', values=['deployments/docker-desktop/attribute_authority-values.yaml']))
k8s_yaml(helm('charts/entitlement', 'entitlement', values=['deployments/docker-desktop/entitlement-values.yaml']))
k8s_yaml(helm('charts/kas', 'kas', values=['deployments/docker-desktop/kas-values.yaml']))
