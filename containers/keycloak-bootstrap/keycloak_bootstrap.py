...
URL_ADMIN_AUTHENTICATOR_EXECUTION_CONFIG = "admin/realms/{realm-name}/authentication/executions/{flow-id}/config"
# We need to do this manually because KeycloakAdmin expects a 204 response and sometimes we get a valid 202.  :/
URL_ADMIN_EXECUTION_FLOW = "admin/realms/{realm-name}/authentication/flows/{flow-alias}/executions"
...
def check_matched(pattern, allData):
    filtered_item = [d for d in allData if all((k in d and d[k] == v) for k, v in pattern.items())]
...
def createUsersInRealm(keycloak_admin):
    ...
    passwordedUsers = os.getenv('passwordUsers', 'testuser@virtru.com,user1,user2').split(',')
...
def addVirtruClientAudienceMapper(keycloak_admin, keycloak_client_id, client_audience):
    ...
    try:
        ...
        logger.warning("Could not add client audience mapper to client %s - this likely means it is already there, so we can ignore this.", keycloak_client_id)
        logger.warning("Unfortunately python-keycloak doesn't seem to have a 'remove-mapper' function")
        logger.warning(str(e))
...
def addVirtruMappers(keycloak_admin, keycloak_client_id):
    ...
    try:
        ...
        logger.warning("Could not add custom userinfo mapper to client %s - this likely means it is already there, so we can ignore this.", keycloak_client_id)
        logger.warning("Unfortunately python-keycloak doesn't seem to have a 'remove-mapper' function")
        logger.warning(str(e))
    try:
        keycloak_admin.add_mapper_to_client(
        ...
        logger.warning("Could not add custom auth mapper to client %s - this likely means it is already there, so we can ignore this.", keycloak_client_id)
        logger.warning("Unfortunately python-keycloak doesn't seem to have a 'remove-mapper' function")
        logger.warning(str(e))
...
def addVirtruDCRSPIREMapper(keycloak_admin, keycloak_client_id):
    ...
    try:
        ...
        logger.warning("Could not add custom spire mapper to client %s - this likely means it is already there, so we can ignore this.", keycloak_client_id)
        logger.warning("Unfortunately python-keycloak doesn't seem to have a 'remove-mapper' function")
        logger.warning(str(e))
...
def createTestClientForClientCredentialsFlow(keycloak_admin, keycloak_auth_url, client_id):
...
def createTestClientForAbacusWebAuth(keycloak_admin):
    ...
    keycloak_admin.create_client(
        payload={
            ...
            "serviceAccountsEnabled": "true",
            "protocol": "openid-connect",
            "redirectUris": ["http://localhost*", "https://demo.secure-analytics-sandbox.virtru.com*"],
            "webOrigins": ["+"],
        },
        skip_exists=True,
    )
...
def assignViewRolesToUser(keycloak_admin, user_id):
    ...
    keycloak_admin.assign_client_role(user_id,
                            realmManagerClient,
                            roles = [viewClients, viewUsers])
...
def createAuthFlowX509(keycloak_admin, realm_name, flow_name, provider_name):
    ...
    if not flow_exist:
        if provider_name == "auth-x509-client-username-form":
            keycloak_admin.copy_authentication_flow(payload={"newName": flow_name}, flow_alias="browser")
        else:
            keycloak_admin.copy_authentication_flow(payload={"newName": flow_name}, flow_alias="direct grant")
    ...
    if filtered_flow:
        payload_config = {"alias": flow_name + "_Config",
                          "config": {
        ...
        conn.raw_post(URL_ADMIN_AUTHENTICATOR_EXECUTION_CONFIG.format(**params_path),
    ...
    if provider_name == "auth-x509-client-username-form":
        ...
        filtered_flow = check_matched({"providerId": "direct-grant-validate-password"}, flows_execution)
        if filtered_flow:
            flow_id = filtered_flow[0]["id"]
            payload_config = {
                "id": flow_id,
            ...
            conn = keycloak_admin.connection
            # We need to do this manually because KeycloakAdmin expects a 204 response and sometimes we get a valid 202.  :/
            conn.raw_put(URL_ADMIN_EXECUTION_FLOW.format(**params_path),
                         data=json.dumps(payload_config))
...
def createTDFRealm(kc_admin_user, kc_admin_pass, kc_url):
    ...
    if npeClientStr is not None:
        for npe_client_id in npeClientStr.split(','):
            createTestClientForClientCredentialsFlow(keycloak_admin, kc_url, npe_client_id)
...
def createTDFPKIRealm(kc_admin_user, kc_admin_pass, kc_url):
    ...
    if not realm_exist:
        logger.info("Create realm %s", realm_name)
        keycloak_admin.create_realm(
            payload={"realm": realm_name, "enabled": "true", "attributes": {"frontendUrl": "http://keycloak-http/auth/realms/tdf-pki"}}, skip_exists=True
        )
    ...
    createAuthFlowX509(keycloak_admin, realm_name, "X509_Browser", "auth-x509-client-username-form")
    ...
    createAuthFlowX509(keycloak_admin, realm_name, "X509_Direct_Grant", "direct-grant-auth-x509-username")
