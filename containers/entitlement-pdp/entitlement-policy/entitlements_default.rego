...
generated_entitlements := entitlements {
...
construct_idp_attribute(attribute_name) = idp_attribute {
    # if idp_context is provided in input return the idp attribute object
    input.idp_context
    input.idp_context[attribute_name]
	not is_null(input.idp_context[attribute_name])
    idp_attribute := {
        "attribute": concat("", ["https://example.org/attr/OPA/value/", attribute_name]),
        "displayName": input.idp_context[attribute_name]
    }
}
