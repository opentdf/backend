...
async def test_function_name(caplog):
    ...
    result = await hook_into(HttpMethod.GET, pre=mock_run_pre_command_hooks_func_name)(
        hi_name
    )("Alice")
...
async def test_sys_exec_info(caplog):
    def mock_run_err_hooks_sys_exec(http_method, function_name, *args, **kwargs):
        err = sys.exc_info()[1]
        logger.info(repr(err))

    caplog.set_level(logging.INFO)
    with pytest.raises(Exception) as e_info:
        result = await hook_into(HttpMethod.GET, err=mock_run_err_hooks_sys_exec)(
            hi_error
        )("Bob")
    messages = [r.msg for r in caplog.records]
