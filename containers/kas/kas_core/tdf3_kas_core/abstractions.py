...
class AbstractHealthzPlugin(AbstractPlugin):
    ...
    def healthz(self, *, probe):
        ...
        logger.warning(
            "AbstractHealthzPlugin method was called with (probe=[%s]) on [%s]",
            probe,
...
class AbstractRewrapPlugin(AbstractPlugin):
    ...
    def fetch_attributes(self, namespaces):
        ...
        """
        logger.warning(
            "AbstractUpsertPlugin fetch_attributes method was called on [%s]",
            self.__class__.__name__,
        )
...
class AbstractUpsertPlugin(AbstractPlugin):
    ...
    def upsert(self, **kwargs):
        """Override this method."""
        logger.warning(
            "AbstractUpsertPlugin upsert method was called on [%s]",
            self.__class__.__name__,
        )
