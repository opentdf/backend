...
def test_add_entitlements_to_entity(test_app, monkeypatch):
    ...
    async def mock_add_entitlements_to_entity_crud(entityId, request, auth_token=None):
        return test_response

    monkeypatch.setattr(
        main, "add_entitlements_to_entity_crud", mock_add_entitlements_to_entity_crud
    )
...
def test_remove_entitlement_from_entity(test_app, monkeypatch):
    ...
    monkeypatch.setattr(
        main,
        "remove_entitlement_from_entity_crud",
        mock_remove_entitlement_from_entity_crud,
    )

    response = test_app.request(
        "DELETE", "/entitlements/tdf_client", data=json.dumps(test_payload)
    )
