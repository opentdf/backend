...
def main():
    ...
    authorization_endpoint = (
        args.oidcEndpoint + "/auth/realms/" + auth[0] + "/protocol/openid-connect/auth"
    )
    ...
    token_endpoint = (
        args.oidcEndpoint + "/auth/realms/" + auth[0] + "/protocol/openid-connect/token"
    )
    ...
    assert response.ok
    # Authorities Create
    authority = {"authority": "https://opentdf.io"}
    ...
    attributes_definitions_endpoint = (
        f"{args.attributesEndpoint}/definitions/attributes"
    )
    ...
    response = requests.post(
        attributes_definitions_endpoint, json=attribute_ip, auth=auth
    )
    print(response.status_code)
    assert response.ok or response.status_code == 400
    attribute_sg = {
    ...
    response = requests.post(
        attributes_definitions_endpoint, json=attribute_sg, auth=auth
    )
    print(response.status_code)
    assert response.ok or response.status_code == 400
    ...
    response = requests.get(
        attributes_definitions_endpoint,
        params={"authority": "https://opentdf.io", "name": "SocialGrade"},
        auth=auth,
    )
    print(response.text)
    assert response.ok
    ...
    response = requests.get(
        attributes_definitions_endpoint,
        params={"authority": "https://opentdf.io", "name": "IntellectualProperty"},
        auth=auth,
    )
    print(response.text)
    assert response.ok
